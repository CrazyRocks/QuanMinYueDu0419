...
{
    UIImage *image;
    image = [OWHTMLToAttriString decryptImage:attachment.contents];
    //[UIImage imageWithContentsOfFile:attachment.contents];
...
//        image   = [imageManager getImageByURL:attachment.contents];
    ...
    if (self.lines.count == 1) {
        ...
        //计算实际图像上边距
        float actualTopDistance = self.frame.size.height - origin.y - attachment.displaySize.height;
        //计算实际最大高度
        float actualHeight = self.frame.size.height - actualTopDistance - self.edgeInsets.bottom - self.edgeInsets.top;
        float actualWidth = attachment.displaySize.width;
        if (actualHeight < attachment.displaySize.height) {
            actualWidth *= (actualHeight/attachment.displaySize.height);
            origin.y = 40;
        }
        else {
            actualHeight = attachment.displaySize.height;
            origin.y -= self.edgeInsets.top;
        }
        origin.x = (self.frame.size.width - actualWidth)/2;
        
        if (origin.x < 0)
            origin.x = 0;
        
        flippedRect = CGRectMake(origin.x, origin.y, actualWidth, actualHeight);
    }
    CGContextDrawImage(context, flippedRect, image.CGImage);
    NSLog(@"%@",NSStringFromCGRect(flippedRect));
    NSLog(@"%@",NSStringFromCGPoint(textPosition));
}
